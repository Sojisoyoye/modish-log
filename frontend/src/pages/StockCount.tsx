import { Container, Typography, Alert, TableContainer, Table, Paper, TableHead, TableRow, TableCell, TableBody, Stack, Button } from "@mui/material";
          ...
          <TableBody>
            {stockCounts.map((stockCount: StockCountItem) => (
              <TableRow key={stockCount.id}>
                <TableCell>{new Date(stockCount.countDate).toLocaleDateString()}</TableCell>
                <TableCell>
                  {stockCount.product.color} ({stockCount.product.size})
                </TableCell>
                <TableCell>{formatNumber(stockCount.countedQuantity)}</TableCell>
                <TableCell>
